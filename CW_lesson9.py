...
# lbp_cascade = cv2.CascadeClassifier('data/lbpcascade_frontalface_improved.xml')
...
while True:
    ...
    # faces = face_cascade.detectMultiScale(gray, scaleFactor = 1.1, minNeighbors = 5, minSize = (30, 30)) # scaleFactor - коефіцієнт масштабування, minNeighbors - кількість перевірок
    # # print(faces)
    # for (x, y, w, h) in faces:
    #     cv2.rectangle(frame, (x,y), (x + w,y + h), (0,255,0), 2)
    #
    #     roi_gray = gray[y:y + h, x:x + w]
    #     roi_color = frame[y:y + h, x:x + w] # зріз для обличчя. передача зрізу для подальшого коду
    #
    ...
    #
    #
    # cv2.putText(frame, f'Faces detected: {len(faces)}', (15, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2)
